...
#'
#'- **`seq_no`**: (integer) Unique patient-visit level identifier (one per row)
#' - **`diag1`**: (character) ICD-10-CM Primary diagnosis code
#' - **`injury_nature_broad`**: (logical) searches all available diagnosis fields on the
#'    hospital discharge record (there can be a maximum of 54 diagnosis fields
#'    in CHARS data)
#' - **`injury_nature_narrow`**: (logical) follows CDC's recommended 'narrow' approach, which
#'    requires that the principal diagnosis of an injury hospitalization be a
#'    nature-of-injury ICD10-CM code
#' - **`injury_intent`**: (character) Injury intent classification, i.e., 'assault', 'intentional',
#'    'legal', 'undetermined', 'unintentional'.
#' - **`injury_mechanism`**: (character) Injury mechanism classification, e.g., 'bites_stings',
#'    'drowning', 'fall', 'firearm', 'overexertion, 'poisoning', etc.
#' - **`chi_geo_kc`**: (character) A King County, WA indicator
#' - **`temperament`**: (character) A categorical 'demographic' indicator
#' - **`creation_date`**: (Date) A simple time stamp of the date the table was updated
#'
#' @source
