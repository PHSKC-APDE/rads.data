...
# Date: April 22, 2022
# Purpose: Create tidy table of CMS 2022 ICD10 CM codes
# Notes: Raw text file read below was originally from https://www.cms.gov/files/zip/2022-code-descriptions-tabular-order-updated-02012022.zip
...
# Function to clean / prep imported data to fix random white spaces (sql_clean) ----
  ...
  # Import full list of ICD 10 CM ----
  icd_icd10cm_codes_2022 = setDT(readr::read_fwf("data-raw/icd_icd10cm_codes_2022.txt",
                                  fwf_widths(c(5,1,7,1,1,1,60,1,200)), show_col_types = FALSE))
  icd_icd10cm_codes_2022 <- icd_icd10cm_codes_2022[, .(order = X1, icd10 = X3, header = X5, short = X7, long = X9)]
  sql_clean(icd_icd10cm_codes_2022)

# Write to package
    usethis::use_data(icd_icd10cm_codes_2022, overwrite = TRUE)
    write.csv(icd_icd10cm_codes_2022, "inst/extdata/icd_data/icd10cm_codes_2022.csv", row.names = F)
